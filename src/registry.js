...
class CommandoRegistry {
	...
	registerEventsIn(path) {
		try {
			...
			for (var i in files) {
				var event = require(path + "/" + files[i]);
				event = event(this.client);
				var name = files[i].replace(".js", "");
				this.events.set(name, event);
				this.client.emit("eventRegister", event, this);
				this.client.emit("debug", `Registered event ${name}.`);
			}
		} catch (error) {
	...
	registerDefaultCommands(commands = {}) {
		...
		if (commands.commandState) {
			if (commands.groups) this.registerCommand(require("./commands/commands/groups"));
			if (commands.enable) this.registerCommand(require("./commands/commands/enable"));
			if (commands.disable) this.registerCommand(require("./commands/commands/disable"));
			if (commands.reload) this.registerCommand(require("./commands/commands/reload"));
			if (commands.load) this.registerCommand(require("./commands/commands/load"));
	...
	findGroups(searchString = null, exact = false) {
		...
		const matchedGroups = Array.from(
			this.groups.filter(exact ? groupFilterExact(lcSearch) : groupFilterInexact(lcSearch)).values()
		);
	...
	findCommands(searchString = null, exact = false, message = null) {
		...
		const matchedCommands = Array.from(
			this.commands
				.filter(exact ? commandFilterExact(lcSearch, message) : commandFilterInexact(lcSearch, message))
				.values()
		);
...
function commandFilterExact(search, msg) {
	return (cmd) => {
		var langCmd = false;
		if (msg.guild) {
...
function commandFilterInexact(search, msg) {
	return (cmd) => {
		var langCmd = false;
		if (msg.guild) {
			var category = msg.guild.lang.commands[cmd.groupID];
