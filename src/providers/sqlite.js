...
class SQLiteProvider extends SettingProvider {
	...
	async init(client, table = "settings") {
		this.client = client;
		await this.db.run(`CREATE TABLE IF NOT EXISTS ${table} (guild INTEGER PRIMARY KEY, settings TEXT)`);

		// Load all settings
		const rows = await this.db.all(`SELECT CAST(guild as TEXT) as guild, settings FROM ${table}`);
		// possible SQL Injection but only by the programmer
		...
		const statements = await Promise.all([
			this.db.prepare(`INSERT OR REPLACE INTO ${table} VALUES(?, ?)`),
			this.db.prepare(`DELETE FROM ${table} WHERE guild = ?`),
		]);
	...
	async destroy() {
		...
		await Promise.all([this.insertOrReplaceStmt.finalize(), this.deleteStmt.finalize()]);
