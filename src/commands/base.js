...
class Command {
	 ...
	 * @property {boolean} [devOnly=false] - Whether or not the command is usable only by the dev
	...
	constructor(client, info) {
		...
		/**
		 * Whether the command can only be used by the dev
		 * @type {boolean}
		 */
		this.devOnly = Boolean(info.devOnly);
		 ...
		 */
		this.argsCollector =
			info.args && info.args.length ? new ArgumentCollector(client, info.args, info.argsPromptLimit) : null;
	...
	hasPermission(message, ownerOverride = true) {
		var lang = message.guild ? message.guild.lang : this.client.en;
		var { owneronly, devonly, missingpermissions, multiplemissingpermissions } = lang.general.commando;

		if (!this.ownerOnly && !this.userPermissions && !this.devOnly) return true;
		if (ownerOverride && this.client.isOwner(message.author) && !this.devOnly) return true;
		if (this.client.isDev(message.author)) return true;
		...
		if (message.channel.type === "text" && this.userPermissions) {
			...
			if (missing.length > 0) {
				if (missing.length === 1) {
					return missingpermissions.replace("{permission}", permissions[missing[0]]);
				}
				return multiplemissingpermissions.replace(
					"{permissions}",
	...
	async run(message, args, fromPattern, result) {
		// eslint-disable-line no-unused-vars, require-await
	...
	onBlock(message, reason, data) {
		...
		var lang = message.guild ? message.guild.lang : message.client.en;
		var {
			guildonly,
			guildonlydesc,
			nopermission,
			...
			botpermissions,
			botpermissionsOne,
			botpermissionsMultiple,
			throttled,
			notnsfw,
		} = lang.general.commando;
		...
		try {
			switch (reason) {
				...
				case "permission": {
					...
					throw nopermissiondesc.replace("{name}", this.name);
				}
				case "clientPermissions": {
					title = botpermissions;
					if (data.missing.length === 1) {
						throw botpermissionsOne
							.replace("{permission}", data.missing.map((perm) => permissions[perm]).join(", "))
							.replace("{name}", this.name);
					}
					throw botpermissionsMultiple
						.replace("{permissions}", permissions[data.missing[0]])
						.replace("{name}", this.name);
				}
				case "throttling": {
					title = throttled;
					throw throttleddesc.replace("{name}", this.name).replace("{seconds}", data.remaining.toFixed(1));
				}
				default:
					return null;
			}
			...
			if (typeof error === "string") {
				message.reply(error, {
					embed: {
						color: "#ff0000",
						author: { name: `Error: ${title}`, icon_url: message.client.savedEmojis.error.url },
					},
				});
			}
		}
	...
	onError(err, message, args, fromPattern, result) {
		var lang = message.guild ? message.guild.lang : message.client.en;
		var { internalerror, reporttodev, fatalerror } = lang.general.commando;
		...
		var errorCode = Math.floor(Math.random() * 90000 + 10000);

		var invites = [];
		if (message.guild) {
			invites = this.client.provider.get(message.guild, "invites") || [];
		}

		var errorMsg = err && err.stack ? err.stack.slice(0, 2000) : err.toString();
		var authorURL = message.guild
			? `https://discord.com/channels/${message.guild.id}/${message.channel.id}/${message.id}`
			: "";

		this.client.treniteLog("``" + message.toString() + "``" + "\n```" + errorMsg + "```\n", {
			...
			embed: {
				...
				footer: {
					...
					icon_url: message.author.displayAvatarURL({ dynamic: true, format: "jpg", size: 256 }),
				},
				color: "#ff0000",
			},
		});
		...
		return message.reply(err.toString(), {
			embed: {
				title: internalerror.replace("{code}", errorCode),
				description: reporttodev,
				color: "#ff0000",
				author: {
					name: fatalerror,
	 ...
	 */
	throttle(userID) {
		if (!this.throttling || /* this.client.isOwner(userID) || */ this.client.isDev(userID)) return null;
	...
	reload() {
		...
		try {
			...
			try {
				...
				if (cached) require.cache[cmdPath] = cached;
				if (err2.message.includes("Cannot find module")) throw err;
				else throw err2;
			}
		}
